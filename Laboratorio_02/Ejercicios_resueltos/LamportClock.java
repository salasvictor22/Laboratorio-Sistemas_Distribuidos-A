...
public class LamportClock {
    ...
    // Se usa cuando ocurre un evento interno
    public synchronized int tick() {
        this.clock++;
        return this.clock;
    }

    // Se usa cuando se recibe un evento/mensaje de otro proceso
    // Ajusta el reloj tomando el valor máximo entre el actual y el recibido
        ...
        for (int i = 0; i < 5; i++) {
            ...
            Thread thread = new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    // Evento local: el hilo genera un evento y aumenta el reloj
                    int time = clock.tick();

                    System.out.println("Thread " + Thread.currentThread().getId() + " created event with Lamport time " + time);

                    try {
                        // Simula el tiempo de espera o retraso en la red
                        Thread.sleep((long) (Math.random() * 1000));
                    } catch (InterruptedException e) {
                        e.printStackTrace();
                    }
                    // Simula la recepción de un evento (otro evento lógico)
            ...
            // Se agrega el hilo a la lista y se inicia su ejecución
            threads.add(thread);
            thread.start();
        }
